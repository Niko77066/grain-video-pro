...
PHOTOS = [  # (dom_id, shot_id, file, kenburns_to)
    ("ph_s06", "s06_housing_photo",    "assets/img/i_s06_housing.jpg",    1.105),
    ("ph_s10", "s10_containers_photo", "assets/img/i_s10_containers.jpg", 1.075),
    ("ph_s12", "s12_equipment_photo",  "assets/img/i_s12_equipment.jpg",  1.085),
]
...
<div class="split">
  <div class="row" data-a="r1"><div class="k">已经跑起来 · 货物进出口</div><div class="v gold">+16.9%</div>
    <div class="bar"><i data-a="rb1" style="background:var(--gold)"></i></div></div>
  <div class="row" data-a="r2"><div class="k">已经跑起来 · 高技术制造业</div><div class="v gold">+13.3%</div>
    <div class="bar"><i data-a="rb2" style="background:var(--gold)"></i></div></div>
  <div class="row" data-a="r3"><div class="k">还没接上 · 社会消费品零售</div><div class="v red">+1.3%</div>
...
for dom, sid, cls, _ in SCENES:
    ...
    if cls == "ph":
        ...
        # 回炉 1（G2 final r1 反模式）：纯 scale 缓推在 3s 内位移太小，被实测判静。
        # 加大缩放并叠一层横向次级位移，让运动在帧差层面真实存在。
        xdrift = -26 if dom != "ph_s10" else 30
        fromto(f"#{dom} [data-a=kb]", "{scale:1.0,x:0}",
               f'{{scale:{kb},x:{xdrift},duration:{dur(sid)},ease:"none"}}', t0(sid))
...
# ---------------------------------------------------------------- HTML
CSS = (P / "compose/style.css").read_text(encoding="utf-8")
_cap_js = (P / "compose/assets/captions_data.js").read_text(encoding="utf-8")
...
"""
(P / "compose/index.html").write_text(html, encoding="utf-8")
