...
3. **风格包**：**跑路由器选，不要凭题材直连映射**（规程 `styles/routing.md`，能力卡 `styles/<pack>/capability.json`）。
   ...
   选定后执行该包的开拍提问模板。当前包：`case-file`（事实核验型新闻解读·竖屏 faceless，已考片）/ `pixel-chronicle`（结构化深度知识叙事·横屏 2–4 分钟，候选）/ `anchor-desk`（官方口径播报型解读·横屏数字人主播台，候选·单片标定）/ `meme-ledger`（授权频道反差画面驱动的商业机制解说·固定 16:9，候选）/ `whiteboard-generalist`（兜底模板，不是第四个风格 SKU）。`_` 前缀目录（`_template` / `_disabled`）不是风格包、不列入。`meme-ledger` 只在 routing 输入包含 `licensed_partner_footage` 且项目已登记视频、声音、BGM 授权引用时可投产。
