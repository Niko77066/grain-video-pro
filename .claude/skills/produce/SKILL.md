...
3. **风格包**：**跑路由器选，不要凭题材直连映射**（规程 `styles/routing.md`，能力卡 `styles/<pack>/capability.json`）。
   ...
   选定后执行该包的开拍提问模板。当前包：`case-file`（事实核验型新闻解读·竖屏 faceless，已考片）/ `pixel-chronicle`（结构化深度知识叙事·横屏 2–4 分钟，候选）/ `anchor-desk`（官方口径播报型解读·横屏数字人主播台，候选·单片标定）/ `whiteboard-generalist`（兜底模板，不是第四个风格 SKU）。`_` 前缀目录（`_template` / `_disabled`）不是风格包、不列入——`meme-ledger` 2026-07-28 退回 `styles/_disabled/`（未打磨完），**不 offer、不路由、不投产**，用户点名要它就说明它在禁用区、复活规程见该目录 README。
